...
from backbone.image_encoder_vgg import ImageClfVGG
from backbone.text_encoder_deberta import DeBERTaClf


class CausalMultimodalModelVGGDeBERTa(nn.Module):
    """
    Causal Representation Learning Model for Multimodal (Image + Text) Learning
    Uses VGG for images and DeBERTa for text
    Implements C3 risk (sufficiency + necessity) for causal inference
    """
    
    def __init__(self, args):
        super(CausalMultimodalModelVGGDeBERTa, self).__init__()
        self.args = args
        
        # Encoders - VGG for images, DeBERTa for text
        self.txtclf = DeBERTaClf(args)
        self.imgclf = ImageClfVGG(args)
        
        # Feature dimension - DeBERTa outputs 768, VGG outputs 4096
        # We'll use DeBERTa dimension (768) as the common dimension
        d = args.hidden_sz  # Should be 768 for DeBERTa
        ...
        # Image projection layer (VGG outputs 4096, need to project to 768)
        ...
        # For VGG: img_hidden_sz = 4096, so if num_image_embeds=1, then img_feat_dim=4096
        # Need to project from 4096 to 768 (d)
        if img_feat_dim != d:
    ...
    def forward(self, txt, mask, segment, img, labels=None):
        ...
        hidden_i = self.imgclf.img_encoder(img)  # [B, num_image_embeds, img_hidden_sz] = [B, 1, 4096]
        hidden_i_flat = torch.flatten(hidden_i, start_dim=1)  # [B, num_image_embeds * img_hidden_sz] = [B, 4096]
        
        # Project image features to same dimension as text (4096 -> 768)
