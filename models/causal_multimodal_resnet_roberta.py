...
from backbone.image_encoder_resnet import ImageClfResNet
from backbone.text_encoder_roberta import RoBERTaClf


class CausalMultimodalModelResNetRoBERTa(nn.Module):
    """
    Causal Representation Learning Model for Multimodal (Image + Text) Learning
    Uses ResNet for images and RoBERTa for text
    Implements C3 risk (sufficiency + necessity) for causal inference
    """
    
    def __init__(self, args):
        super(CausalMultimodalModelResNetRoBERTa, self).__init__()
        self.args = args
        
        # Encoders - ResNet for images, RoBERTa for text
        self.txtclf = RoBERTaClf(args)
        self.imgclf = ImageClfResNet(args)
        
        # Feature dimension - RoBERTa outputs 768, ResNet50 outputs 2048
        # We'll use RoBERTa dimension (768) as the common dimension
        d = args.hidden_sz  # Should be 768 for RoBERTa
        ...
        # Image projection layer (ResNet50 outputs 2048, need to project to 768)
        ...
        # For ResNet50: img_hidden_sz = 2048, so if num_image_embeds=1, then img_feat_dim=2048
        # Need to project from 2048 to 768 (d)
        if img_feat_dim != d:
    ...
    def forward(self, txt, mask, segment, img, labels=None):
        ...
        hidden_i = self.imgclf.img_encoder(img)  # [B, num_image_embeds, img_hidden_sz] = [B, 1, 2048]
        hidden_i_flat = torch.flatten(hidden_i, start_dim=1)  # [B, num_image_embeds * img_hidden_sz] = [B, 2048]
        
        # Project image features to same dimension as text (2048 -> 768)
