...
def get_args():
    parser = argparse.ArgumentParser(description="Train Causal Multimodal Model (EfficientNet + DistilBERT) on FairCLIP")
    ...
    # Model arguments
    parser.add_argument("--distilbert_model", type=str, default="distilbert-base-uncased",
                        help="DistilBERT model name")
    parser.add_argument("--efficientnet_variant", type=str, default="efficientnet_b0",
    ...
    parser.add_argument("--hidden_sz", type=int, default=768,
                        help="Hidden size (DistilBERT output dimension)")
    parser.add_argument("--img_hidden_sz", type=int, default=1280,
                        help="Image encoder hidden size (EfficientNet-B0 outputs 1280)")
    ...
    parser.add_argument("--name", type=str, default="causal_efficientnet_distilbert",
...
def model_forward(model, batch, args, compute_causal=True):
    ...
    if compute_causal and labels is not None:
        ...
        fusion_logits, txt_logits, img_logits = model(txt, mask, segment, img, labels=None)
        return {
            'fusion_logits': fusion_logits,
            'txt_logits': txt_logits,
            'img_logits': img_logits,
...
def main():
    ...
    # Create model
    logger.info("Creating model with EfficientNet + DistilBERT...")
    model = CausalMultimodalModelEfficientNetDistilBERT(args)
    model.cuda()
    logger.info(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")
    
    # Optimizer with different learning rates for encoders vs new layers
    # Use smaller LR for pretrained encoders, larger for new fusion layers
    encoder_params = []
    new_params = []
